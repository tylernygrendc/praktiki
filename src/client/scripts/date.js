...
function daysIn(month = new Date().getMonth()){
    try{
        if(typeof month === "number"){
            ...
            throw new Error(`Error: function daysIn() requires parameter type of "number". Try passing new Date(year, month).getMonth().`)
        }
    } 
    catch(err) {
        error.handle(err);
...
function getMonthArray(date = new Date()){
    try {
        if(typeof date === "object"){
            ...
            throw new Error(`Error: function getCalendarArrayFor() requires parameter type of "object". Try passing new Date().`)    
        }
    }
    catch(err){
        error.handle(err);
...
function incrementMonth(date) {
    try {
        if(typeof month === "object"){
            ...
            throw new Error(`Error: function decrementMonth() requires parameter type of "object". Try passing new Date().`);
        }
    }
    catch (err) {
        error.handle(err);
